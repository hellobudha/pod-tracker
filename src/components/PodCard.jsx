...
export default function PodCard({ pod, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left bg-white dark:bg-gray-800 rounded-2xl px-4 py-3 shadow-sm active:scale-[0.98] transition-transform ${cardBorder}`}
    >
        ...
        <div className="flex gap-4 mt-2">
          ...
          {pod.reorder && (
            <div className="ml-auto flex items-end">
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold">↻ Reorder</span>
            </div>
          )}
        </div>
      )}
